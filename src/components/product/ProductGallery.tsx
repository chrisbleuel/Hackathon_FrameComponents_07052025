...
export const ProductGallery: React.FC<ProductGalleryProps> = ({
  ...
  return <div className={`${isMobile ? 'w-full' : 'min-w-60 w-[471px]'} bg-[#F5F8FA] rounded-t-lg`}>
      <div className="flex w-full items-stretch p-4 md:p-6 rounded-md px-0 bg-transparent py-0">
          ...
          <div className="w-full pb-2">
            ...
            <div className="w-full mt-2 px-4 md:px-16">
              <div className="flex w-full items-stretch gap-[5px]">
                {images.map((_, idx) => <button key={idx} onClick={() => setCurrentIndex(idx)} className={`flex ${idx === currentIndex ? 'w-[60px] md:w-[95px] bg-[#202327]' : 'w-[120px] md:w-[190px] bg-[#E6EBF0]'} shrink-0 h-1`} aria-label={`View image ${idx + 1}`} />)}
              </div>
            </div>
          </div>
        </div>
